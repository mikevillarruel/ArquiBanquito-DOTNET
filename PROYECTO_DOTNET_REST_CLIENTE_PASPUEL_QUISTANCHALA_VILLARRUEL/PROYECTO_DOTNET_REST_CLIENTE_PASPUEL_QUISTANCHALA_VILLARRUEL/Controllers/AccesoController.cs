using PROYECTO_DOTNET_REST_CLIENTE_PASPUEL_QUISTANCHALA_VILLARRUEL.Filters;
...
namespace PROYECTO_DOTNET_REST_CLIENTE_PASPUEL_QUISTANCHALA_VILLARRUEL.Controllers
{
    public class AccesoController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> Login(string nombreUsuario, string contrasenia)
        {
            contrasenia = Encriptar(contrasenia);
            CoreBancarioService service = new CoreBancarioService();
            if (await service.inicioSesion(nombreUsuario, contrasenia))
            {
                Usuario us = await service.obtenerUsuario(nombreUsuario);
                Session["Usuario"] = us;
        ...
        [HttpPost]
        public async Task<ActionResult> ActualizarContrasenia(String nuevaContrasenia, String nuevaContraseniaR)
        {
            CoreBancarioService service = new CoreBancarioService();
            ...
            if (nuevaContrasenia.Equals(nuevaContraseniaR))
            {
                if (us != null)
                {
                    if (await service.actualizarContrasena(us.nombre_usuario, nuevaContrasenia))
                    {
                        Session["Usuario"] = await service.obtenerUsuario(us.nombre_usuario);
                        return RedirectToAction("Index", "Home");
                    }
                    else
                    {
        ...
        [HttpPost]
        [VerificaSession(Disable = true)]
        public async Task<ActionResult> RegistrarUsuario(String cedula, String nroCuenta)
        {
            if (ValidarCedula(cedula)){
                CoreBancarioService service = new CoreBancarioService();
                Cliente cl = await service.verificarCliente(cedula, nroCuenta);
                if (cl.id_cliente != 0)
                {
                    return RedirectToAction("Login", "Acceso");
                }
